...
createApp({
  data() {
    return {
      ...
      technologies: ['HTML5 semântico','CSS3 responsivo','Vue.js 3','JavaScript ES2022','Visual Studio Code','Git/GitHub']
    };
  },
  computed: {
  ...
  methods: {
    ...
    submitAppointment() {
      ...
      const appointment = {...this.booking, id: Date.now(), date:d, status:'Confirmado'};
    ...
    requestCancel(a) {
      this.modal = {visible:true,title:'Cancelar atendimento?',body:`Você está solicitando o cancelamento de <strong>${a.type}</strong> em <strong>${a.date}</strong> às <strong>${a.time}</strong>.`,confirm:()=>{a.status='Cancelado';this.closeModal();this.flash('Atendimento cancelado.');}};
    },
    openNotification(n) { n.read = true; this.modal = {visible:true,title:n.title,body:`${n.text}<br><small>${n.time}</small>`,confirm:null}; },
    markAllRead() { this.notifications.forEach(n=>n.read=true); this.flash('Notificações marcadas como lidas.'); },
    submitSupport() { this.support={subject:'',message:''}; this.flash('Mensagem registrada na demonstração.'); }
